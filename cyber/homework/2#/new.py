...
def both_ends(my_input):
    ...
    if len(my_input) < 2:
        return ""
    return my_input[0:2] + my_input[len(my_input)-2:len(my_input)]
...
def fix_start(my_input):
    """ +++your code here+++ """
    first = my_input[0]  # Saves first char to add it after replacing all instance of him to *
    return first + my_input.replace(my_input[0], "*")[1:]
